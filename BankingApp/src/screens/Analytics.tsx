...
export function Analytics() {
  ...
  const { data: monthlyData } = useQuery({ queryKey: ["analytics-monthly"], queryFn: api.getAnalyticsMonthly });
  const { data: categoriesData } = useQuery({ queryKey: ["analytics-categories"], queryFn: api.getAnalyticsCategories });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Activity className="h-4 w-4" /> Income vs Expenses</CardTitle></CardHeader>
          <CardContent>
            {monthly.length === 0 ? (
